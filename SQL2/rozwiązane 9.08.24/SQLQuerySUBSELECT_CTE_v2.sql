...
-- ** ¿eby uzyskaæ to samo dodajemy funkcjê rankingow¹ i pobieramy tylko wynik z rankingiem 1
SELECT * FROM
tblKlienci k
JOIN tblPracownicy p  ON p.Miasto = k.Miasto 
...
-- wersja z CTE
;WITH 
NajdrozszyTowar AS (      -- œrednik przed WITH nie jest konieczny, ale bez niego nie odpalimy poprzedniego zapytania razem z tym
...
-- tabela tymczasowa 
CREATE TABLE #NajdrozszyTowar
(
	MaxCena NUMERIC
)
INSERT INTO #NajdrozszyTowar
SELECT MAX(cena) AS MaxCena
    FROM tbTowary
...
DROP TABLE #NajdrozszyTowar


-- wersja z subselect
USE HM
SELECT cena , wykonawca + ' - ' + tytul, (SELECT AVG(cena) FROM tbTowary)
FROM tbTowary
WHERE cena > (SELECT AVG(cena) FROM tbTowary);
...
;WITH NajdrozszyTowar AS (      -- œrednik przed WITH nie jest konieczny, ale bez niego nie odpalimy poprzedniego zapytania razem z tym
    SELECT AVG(cena) AS AvgCena
    FROM tbTowary
)
SELECT cena, wykonawca + ' - ' + tytul, (SELECT AvgCena FROM NajdrozszyTowar)
FROM tbTowary
WHERE cena > (SELECT AvgCena FROM NajdrozszyTowar)
