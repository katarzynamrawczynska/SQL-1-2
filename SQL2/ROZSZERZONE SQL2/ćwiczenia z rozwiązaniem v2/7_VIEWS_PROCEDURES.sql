...
/*Procedura z dynamicznym SQL - problemy które rozwi¹zuje*/
BEGIN TRANSACTION
...
BEGIN
    ...
    FROM tbKlienci k
    JOIN StatystykiKlienta sk ON k.IDKlienta = sk.KlientID
	-- ORDER BY @NazwaKolumny -- @KierunekSortowania -- to samo z desc/asc
END;
GO
...
SET @SQLQuery =  N'WITH StatystykiKlienta AS (
    ...
    FROM tbKlienci k
    JOIN StatystykiKlienta sk ON k.IDKlienta = sk.KlientID
ORDER BY ' + @NazwaKolumny + ' ' + @KierunekSortowania;
...
-- 2. Przy u¿yciu Dynamic SQL stwórz procedurê, która dostaje nazwê kolumny po której powinna partycjonowaæ wynik zapytania
-- poni¿ej i umieœæ j¹ w partycjonowaniu w funkcji okna oraz w sortowaniu. W przyk³adzie zapytania u¿yto kolumny noœnik

SELECT wykonawca, tytul, cena, dystrybutor, nosnik,
DENSE_RANK() OVER (PARTITION BY nosnik ORDER BY cena) AS d_rank,
RANK() OVER (PARTITION BY nosnik ORDER BY cena) AS rank
FROM tbTowary
ORDER BY nosnik


CREATE OR ALTER PROCEDURE AlbumsRank @NazwaKolumny NVARCHAR(128)
AS
BEGIN
DECLARE @SQLQuery AS NVARCHAR(MAX); -- zmienna w której skleimy zaptanie
SET @SQLQuery = 'SELECT wykonawca, tytul, cena, dystrybutor, nosnik,
DENSE_RANK() OVER (PARTITION BY '+@NazwaKolumny+' ORDER BY cena) AS d_rank,
RANK() OVER (PARTITION BY '+@NazwaKolumny+' ORDER BY cena) AS rank
FROM tbTowary
