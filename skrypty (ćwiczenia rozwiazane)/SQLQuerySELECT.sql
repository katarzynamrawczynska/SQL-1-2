
USE BS
GO
/* SELECT - PODSTAWY */

-- nie nadu¿ywaj gwazdki, bo poczekasz d³u¿ej
SELECT TOP 3 *
FROM tblKategorie
ORDER BY NazwaKategorii; -- DESC 

SELECT ID_Kategoria AS identyfikator, -- AS nie jest konieczne
	NazwaKategorii AS "nazwa kategorii"
FROM tblKategorie;

SELECT * FROM tblPracownicy
ORDER BY Miasto DESC, Nazwisko ASC; -- asc nie jest konieczne

SELECT DISTINCT TOP 4 Stanowisko innanazwa
FROM tblPracownicy;



/*æwiczenia*/
-- 1. Kody klientów i ich nazwy, posortowane malej¹co po kodzie, ograniczone do pierwszych 5
SELECT TOP 5 KodKlienta AS [KOD KLIENTA], NazwaFirmy AS [NAZWA]
FROM tblKlienci
ORDER BY KodKlienta DESC;

-- 2. Nazwy miast z siedzibami klientów (bez duplikatów)
SELECT DISTINCT Miasto
FROM tblKlienci;

-- 3. Wszystkie dane z tblOpisSprzedazy, posortowane po iloœci oraz cenie sprzeda¿y rosn¹co
SELECT *
FROM tblOpisSprzedazy
ORDER BY Ilosc ASC, CenaSprzedazy ASC;


/*WARUNEK WHERE*/

SELECT * FROM tblKlienci
WHERE TelefonKomorkowy IS NULL AND Miasto = 'Warszawa';


SELECT NazwaTowaru, Kategoria_ID , Cena_Katalogowa
FROM tblTowary
WHERE 
	Cena_Katalogowa >= 6 AND Cena_Katalogowa <= 10 -- TODO u¿yjmy BETWEEN
	-- OR Kategoria_ID != 6 

SELECT NazwaTowaru, Kategoria_ID , Cena_Katalogowa
FROM tblTowary
WHERE 
	Cena_Katalogowa < 6 AND Cena_Katalogowa > 10 -- TODO u¿yjmy NOT BETWEEN



SELECT NazwaTowaru AS Towar, Kategoria_ID , Cena_Katalogowa
FROM tblTowary
WHERE Kategoria_ID = 1 OR 
	Kategoria_ID = 3 AND 
	Cena_Katalogowa > 6 -- TODO zmieñmy nawiasowanie
ORDER BY Cena_Katalogowa;

-- TODO zmieñmy na wersjê z operatorem IN (kategorie)
--


SELECT Imie, Nazwisko, Miasto
FROM tblPracownicy
WHERE Nazwisko like 'k%'

SELECT Imie, Nazwisko, Miasto
FROM tblPracownicy
WHERE Nazwisko like '_o%' -- % dowolny ciag, _ dowolny jeden znak

SELECT Imie, Nazwisko, Miasto
FROM tblPracownicy
WHERE Nazwisko like '[a-k]%' OR Nazwisko LIKE '%S%'
ORDER BY Nazwisko


/* æwieczenia */
-- 1.Znajdz w pracownika ktory pracuje na stanowisku Przedstawiciel handlowy
-- 2. wyswietl kod klienta, nazwê firmy, ulice oraz miasto z tabeli klineci. Wybierz tych,
--		ktorzy mieszkaja w miescie na W i kod klienta jest dwuznakowy
--		posortuj wg kodu malejaco
-- 3. Wybierz klientów, których telefon nie ma "2" jako drug¹ cyfrê, oraz nazwa firmy zawiera literê "a"
-- 4. Z tabeli pracowników wybierz wszystkie imiona i nazwiska osób których id mieœci siê miêdzy 5 a 12 
--		lub nazwa ich stanowiska zawiera w sobie s³owo "handlowy" 
-- 5. Wybierz z tabeli pracownicy osoby o inicja³ach M.S. i J.M.





/* FUNKCJE SKALARNE */
SELECT 
LTRIM(NazwaTowaru), ID_Towar + Kategoria_ID
FROM tblTowary
WHERE LTRIM(NazwaTowaru) like 'Czekolada%'
ORDER BY ID_Towar + Kategoria_ID;

SELECT 
ROUND(15.193, 1) AS zaokr¹glenie,  
POWER(3, 2) as potêga,  
FLOOR(15.7) AS pod³oga,  
CEILING(15.7) AS sufit;

SELECT 
LEN('Hello World') AS d³ugoœæ_napisu,
LEFT('Hello World', 5) AS lewa_czêœæ_napisu,
RIGHT('Hello World', 5) AS prawa_czêœæ_napisu,
SUBSTRING('Hello World',1, 3) AS wycinek,
REPLACE('Hello World', 'World', 'DB2') AS napis_z_zast¹pieniem,
UPPER('Hello World') AS wielkie_litery,
LOWER('Hello World') AS ma³e_litery;

SELECT (Imie + ' ' + Nazwisko) AS pe³ne_imiê FROM tblPracownicy; 

SELECT NazwaFirmy,
		Telefon,
		COALESCE(TelefonKomorkowy, 'brak numeru komórkowego') AS TelefonKomorkowy
FROM tblKlienci;

SELECT 
CAST(GETDATE() AS DATE) AS aktualna_data, 
CAST(GETDATE() AS TIME) AS aktualny_czas,
'Data: ' + CAST(GETDATE() AS VARCHAR) AS data_napis,
GETDATE() AS aktualna_data_i_czas,
MONTH(GETDATE()) AS miesi¹c, -- to samo z YEAR, DAY
DATENAME(year, GETDATE()) AS rok,
DATENAME(day, GETDATE()) AS dzien,
DATENAME(weekday, GETDATE()) AS nazwa_dnia,
DATEPART(weekday, GETDATE()) AS numer_dnia_tygodnia,
CAST('2023-03-15' AS DATE) AS zmiana_napisu_na_datê;

SELECT 
ID_Sprzedaz, 
DATEDIFF(day, DataSprzedazy, GETDATE()) AS roznica_dni,
DATEDIFF(month, DataSprzedazy, GETDATE()) AS ró¿nica_miesiêcy  
FROM tblSprzedaz;

/*æwiczenia*/
-- 1. ZnajdŸ pracownika na stanowisku Przedstawiciel handlowy
SELECT *
FROM tblPracownicy
WHERE Stanowisko = 'Przedstawiciel handlowy';

-- 2. Wyœwietl kod klienta, nazwê firmy, ulicê oraz miasto klientów mieszkaj¹cych w mieœcie na 'W' i maj¹cych dwuznakowy kod klienta
SELECT KodKlienta, NazwaFirmy, Ulica, Miasto
FROM tblKlienci
WHERE Miasto LIKE 'W%' AND KodKlienta LIKE '__'
ORDER BY KodKlienta DESC;

-- 3. Wybierz klientów, których telefon nie ma "2" jako drug¹ cyfrê, oraz nazwa firmy zawiera literê "a"
SELECT *
FROM tblKlienci
WHERE Telefon NOT LIKE '_2%' AND NazwaFirmy LIKE '%a%';

-- 4. Wybierz imiona i nazwiska pracowników o ID miêdzy 5 a 12 lub ze stanowiskiem zawieraj¹cym "handlowy"
SELECT Imie, Nazwisko
FROM tblPracownicy
WHERE IDPracownika BETWEEN 5 AND 12 OR Stanowisko LIKE '%handlowy%';

-- 5. Wybierz z tabeli pracownicy osoby o inicja³ach M.S. i J.M.
SELECT *
FROM tblPracownicy
WHERE (Imie LIKE 'M%' AND Nazwisko LIKE 'S%') OR (Imie LIKE 'J%' AND Nazwisko LIKE 'M%');



/*FETCH OFFSET*/
-- Wybieranie z wyniku wierszy 21-30 
SELECT *
FROM tblPracownicy
ORDER BY Nazwisko
OFFSET 5 ROWS        -- Pomiñ pierwsze 5 wierszy
FETCH NEXT 10 ROWS ONLY; -- Nastêpnie pobierz 10 wierszy

/*FETCH, OFFSET, CASE */

-- Wybieranie z wyniku wierszy 21-30 
SELECT *
FROM tblPracownicy
ORDER BY Nazwisko
OFFSET 5 ROWS        -- Pomiñ pierwsze 5 wierszy
FETCH NEXT 10 ROWS ONLY; -- Nastêpnie pobierz 10 wierszy


/*CASE - wyœwietl wed³ug warunków*/

SELECT  ID_OpisSprzedazy, 
	Towar_ID, 
	CASE
		WHEN Ilosc <= 5 THEN 'Pojedyncza'
		WHEN Ilosc <= 15 THEN 'Detaliczna'
		ELSE 'Hurtowa'
	END AS typSprzedazy
FROM tblOpisSprzedazy


/*æwiczenia*/
-- 1. Wyœwietl dane z tabeli tblOpisSprzedazy posortowane po iloœci oraz cenie sprzeda¿y (oba rosn¹co). Ogranicz wyniki 
	-- do takich, gdzie cena sprzeda¿y jest równa 3, g lub 5, lub zawiera siê miêdzy 10, a 25. 
	-- Nastêpnie z posortowanych danych wybierz 10 wiersy, od 11-20 (OFFSET, FETCH)

-- 2. Wybierz z tabeli tblSprzedaz id klienta, rok sprzeda¿y, miesi¹c sprzeda¿y, oraz dodatkowe pole które zawiera:
 -- Jeœli rok sprzedarzy to 2017 wypisz 'Faktura tegoroczna',
 -- Jeœli rok sprzeda¿y to 2016, to 'Faktura zesz³oroczna',
 -- W innym przypadku 'Starsza'


 /*æwiczenia dodatkowe */
-- 1. wyœwietl nazwy i ceny towarów posortowane po cenie malej¹co. W wyniku pomiñ pierwsze 3 wiersze i poka¿ kolejne 7 (trzeba u¿yæ JOIN)
-- 2. Z tabeli pracowników wybierz wszystkie imiona i nazwiska. Jeœli nazwa stanowiska zawiera w sobie s³owo "handlowy" 
-- przy nazwisku wyœwietl napis 'Handlowiec', w innym przypadku wyœwietl napis 'Inne stanowisko'

-- 1. Wyœwietl dane z tblOpisSprzedazy posortowane po iloœci oraz cenie sprzeda¿y
SELECT *
FROM tblOpisSprzedazy
WHERE CenaSprzedazy IN (3, 5) OR (CenaSprzedazy BETWEEN 10 AND 25)
ORDER BY Ilosc ASC, CenaSprzedazy ASC
OFFSET 10 ROWS FETCH NEXT 10 ROWS ONLY;

-- 2. Wybierz z tblSprzedaz id klienta, rok sprzeda¿y, miesi¹c sprzeda¿y i status faktury
SELECT 
    Klient_ID,
    YEAR(DataSprzedazy) AS RokSprzedazy,
    MONTH(DataSprzedazy) AS MiesiacSprzedazy,
    CASE 
        WHEN YEAR(DataSprzedazy) = 2017 THEN 'Faktura tegoroczna'
        WHEN YEAR(DataSprzedazy) = 2016 THEN 'Faktura zesz³oroczna'
        ELSE 'Starsza'
    END AS StatusFaktury
FROM tblSprzedaz;

-- Æwiczenia dodatkowe 1: Wyœwietl nazwy i ceny towarów posortowane po cenie malej¹co
SELECT NazwaTowaru, Cena_Katalogowa
FROM tblTowary
ORDER BY Cena_Katalogowa DESC
OFFSET 3 ROWS FETCH NEXT 7 ROWS ONLY;

-- Æwiczenia dodatkowe 2: Wybierz imiona i nazwiska pracowników z okreœleniem typu stanowiska
SELECT 
    Imie, 
    Nazwisko,
    CASE 
        WHEN Stanowisko LIKE '%handlowy%' THEN 'Handlowiec'
        ELSE 'Inne stanowisko'
    END AS TypStanowiska
FROM tblPracownicy;
